...
#define CTEST_EXPECT(msg, condition)                             \
    do {                                                         \
        ...
        if (!(condition)) {                                      \
            printf(                                              \
                "    [  \x1B[31mFAIL\x1B[0m  ] %s\n\n"           \
                "        Failed assertion \x1B[31m%s\x1B[0m at " \
                "\x1B[36m%s:%u\x1B[0m\n",                        \
...
#define CTEST_BENCH_END(size)                                                  \
    do {                                                                       \
        ...
        ctest_elapsed =                                                        \
            (double)(ctest_end.tv_sec - ctest_start.tv_sec) +                  \
            (double)(ctest_end.tv_usec - ctest_start.tv_usec) / 1000000;       \
        if (size / ctest_elapsed / 1024 / 1024 >= 10000) {                     \
            printf(                                                            \
                "\r    [  \x1B[32m%1.2fs\x1B[0m  \x1B[33minf MB/s\x1B[0m  ] "  \
                "%s",                                                          \
                ctest_elapsed, ctest_msg);                                     \
        } else {                                                               \
            printf(                                                            \
                "\r    [  \x1B[32m%1.2fs\x1B[0m  \x1B[33m%4.0fMB/s\x1B[0m  ] " \
                "%s",                                                          \
                ctest_elapsed, (float)(size) / 1024 / 1024 / ctest_elapsed,    \
                ctest_msg);                                                    \
        }                                                                      \
        printf("                        \n");                                  \
    } while (0)
