...
class TestStructures(TestCase):
    ...
    def test_array_append(self):
        ...
        self.assertEqual(s.values(), [1, 2, 3, 4, 10])
        s.free()
    ...
    def test_array_buffer_offset(self):
        ...
        self.assertEqual(copy.values(), [2, 3])
        copy.free()

    def test_map_empty_map(self):
        v = SparseMap()
        self.assertEqual(dict(v.items()), {})
    ...
    def test_map_append(self):
        ...
        v.append(1, s)
        s.free()
        self.assertEqual(dict(v.items()), {1: [1, 2, 3]})
        v.free()
    ...
    def test_map_import_export(self):
        ...
        self.assertEqual(dict(copy.items()),
                         {1: [1, 2, 3, 4], 3: [2, 4, 6, 8]})
        copy.free()
        v = SparseMap()
        self.assertIsNone(v.buffer())
        v.free()
    ...
    def test_map_buffer_offset(self):
        ...
        v.free()
        copy = SparseMap(buf, len(buf))
        self.assertEqual(dict(copy.items()), {3: [1, 2, 3]})
        copy.free()
    ...
    def test_context_manager_free(self):
        ...
        with FloatArray() as structure:
            outside = structure
            self.assertIsNotNone(structure.ptr)
    ...
    def test_map_repr(self):
        ...
        b = FloatArray([4])
        m.append(2, b)
        self.assertEqual(str(m), '[ 1 ] = [1.0, 2.0, 3.0]\n[ 2 ] = [4.0]')
        a.free()
        b.free()
        m.free()
