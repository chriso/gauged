...
class TestDriver(TestCase):
    ...
    def test_insert_keys(self):
        ...
        self.assertIsNone(ids[(1, 'foobar')])
        self.assertEqual(len(ids), 3)
        ids = self.driver.lookup_ids([(2, 'foo'), (2, 'foobar')])
        self.assertIsNone(ids[(2, 'foo')])
    ...
    def test_insert_blocks(self):
        ...
        self.assertEqual(str(self.driver.get_block(0, 1, 2)[0]),
                         '\xe4\x00\x12')
        self.driver.insert_or_append_blocks([(0,  1, 2, '\xe4\x00\x12', 0x10)])
        self.assertEqual(str(self.driver.get_block(0, 1, 2)[0]),
                         '\xe4\x00\x12\xe4\x00\x12')
    ...
    def test_keys(self):
        ...
        self.assertEqual(keys, ['bar', 'foobar', 'fooqux'])
        keys = self.driver.keys(1, prefix='foo')
        self.assertEqual(keys, ['foobar', 'fooqux'])
    ...
    def test_remove_namespace(self):
        ...
        self.driver.remove_namespace(1)
        self.assertEqual(str(self.driver.get_block(0, 1, 2)[0]), 'foo')
        self.assertIsNone(self.driver.get_block(1, 2, 3)[0])
    ...
    def test_cache(self):
        ...
        self.assertSequenceEqual(self.driver.get_cache(1, id_, 2, 3, 4),
                                 ((3, 4),))
    ...
    def test_namespace_statistics(self):
        ...
        self.assertIsNone(min_block)
        self.assertIsNone(max_block)
        statistics = self.driver.get_namespace_statistics(0, 0, 0)
        self.assertEqual(statistics, [0, 0])
        ...
        statistics = self.driver.get_namespace_statistics(0, 0, 0)
        self.assertEqual(statistics, [7, 9])
        statistics = self.driver.get_namespace_statistics(0, 0, 1)
        self.assertEqual(statistics, [108, 111])
        min_block, max_block = self.driver.block_offset_bounds(0)
    ...
    def test_clear_from(self):
        ...
        self.driver.replace_blocks(blocks)
        id_ = sha1('foobar').digest()
        self.driver.add_cache(0, 9, id_, 5,
                              [(13, 4), (15, 4), (18, 4), (23, 6)])
        ...
        self.assertSequenceEqual(self.driver.get_cache(0, id_, 5, 0, 100),
                                 [(13, 4)])
        self.assertEqual(self.driver.get_namespace_statistics(1, 0, 3),
                         [4, 5])

    def test_metadata(self):
        self.assertIsNone(self.driver.get_metadata('foobaz'))
        ...
        self.driver.set_metadata({'foo': 'bar', 'qux': 'foobar'},
                                 replace=False)
        self.assertEqual(self.driver.get_metadata('foo'), 'qux')
